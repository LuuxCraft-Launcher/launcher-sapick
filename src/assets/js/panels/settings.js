...
class Settings {
    ...
    async javaPath() {
        ...
        javaPathInputFile.addEventListener("change", async () => {
            let selectedFile = javaPathInputFile.files[0];
            if (!selectedFile) return;

            let file = webUtils?.getPathForFile?.(selectedFile) || selectedFile.path;
            if (!file) return alert("Impossible de recuperer le chemin du fichier Java.");

            let fileName = (file || selectedFile.name || javaPathInputFile.value).split(/[\\/]/).pop().replace(/\.exe$/i, '').toLowerCase();
